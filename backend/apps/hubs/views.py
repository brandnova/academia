...
class RejectActivationRequestView(APIView):
    ...
    def post(self, request, request_id):
        ...
        # Accepted per api-contract.md but not persisted anywhere, see the
        # deviation note in chat: no HubActivationRequest field exists to
        # store an admin's rejection reason today. Used here for the email
        # only, then discarded, same as before this change.
        ...
        notify(
            user=activation_request.user,
            notification_type=Notification.Type.HUB_ACTIVATION_REJECTED,
            message=f"Your hub activation request for {activation_request.school.short_name} was not approved",
            content_object=activation_request,
            email_subject="Your hub activation request was not approved",
            email_template="hub_activation_rejected",
            email_context={
                "recipient_name": activation_request.user.full_name,
                "school_name": activation_request.school.name,
                "reason": reason,
            },
        )
