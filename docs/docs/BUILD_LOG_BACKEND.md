...
## View Count Dedup Fix (Post-MVP)
Not a numbered phase. Bug: view_count was documented and implemented as
deduplicated per authenticated user for a rolling 24-hour window, but was
observed incrementing on every refresh in practice.

## School Directory Import (Post-MVP)
Not a numbered phase. Imports a real Nigerian tertiary institution directory
(NUC universities, NBTE polytechnics, NCCE colleges of education, 1,357 raw
source entries) to replace the placeholder-only School table, closing out
project-plan.md's Developer Tooling & Data Sourcing item and feature-list.md's
School Data Curation sourcing line.
...
- Notification message includes the report's type (spam/abuse/etc.,
  ...
  including it
- QuestionDetailView.get()'s view-count dedup now routes through
  apps.core.cache's get_cached/set_cached instead of importing Django's
  cache module directly, so a cache read/write failure here logs a warning
  like every other cache use in the codebase, instead of failing silently
- Cache key now uses the academia:v1: prefix, matching every other cache
  key generated by make_cache_key/make_query_cache_key, previously this
  one key shape was the sole exception
- School gained institution_type (UNIVERSITY/POLYTECHNIC/COLLEGE_OF_EDUCATION),
  ownership (FEDERAL/STATE/PRIVATE), state, regulatory_code, source_url,
  last_verified_at, and country (default "Nigeria", forward-compatible for
  eventual multi-country expansion, no multi-country logic exists yet)
- New SchoolSourceRecord model: per-regulator provenance (regulator,
  raw_category, raw_payload JSON, source_url, fetched_at, is_current),
  FK to School, admin-visible inline. Exists specifically so a future
  re-verification pass can flip is_current False without ever
  auto-deactivating the School itself
- import_school_directory management command, idempotent, matches existing
  schools by short_name then name, only fills currently-blank fields, never
  overwrites a value already set. Newly created schools import with
  is_active=False, requiring individual admin review before going live,
  matched/existing schools keep their current is_active unchanged
- known_possible_duplicates.csv committed alongside the import data
...
- Root cause was very likely a Redis connectivity issue combined with
  IGNORE_EXCEPTIONS=True: a cache.get() against an unreachable Redis
  instance returns None rather than raising, which is indistinguishable
  from "genuinely not cached yet," so the dedup check always took the
  increment branch. This is the correct fail-open behavior for
  throttling and read-heavy view caching, but for THIS specific use
  (correctness of a stored count, not just performance), a silent failure
  actively corrupts data rather than just degrading gracefully. The fix
  doesn't change the fail-open behavior itself, that's still the right
  call platform-wide, it makes the failure visible via logging instead
- Did not add alerting/monitoring on this specific log line, that's
  general infrastructure work (Sentry or similar), already noted in the
  production-readiness durability list from an earlier pass, not scoped
  into this fix
- School Data Curation groundwork landed ahead of the actual NUC/NBTE/NCCE
  import: institution_type/ownership/state/regulatory_code/source_url/
  last_verified_at added to School, all nullable so this was a zero-impact
  migration for existing data. SchoolSourceRecord added alongside for
  per-regulator provenance, lives in apps.schools (not a standalone app),
  matching the established pattern for sub-resources with no independent
  lifecycle. last_verified_at is a plain field set explicitly by whatever
  process last confirmed the record, not signal-derived, consistent with
  how School.slug is already generated once at creation rather than kept
  in sync automatically
- country added to School now, while the directory is still seed-only, to
  avoid a much more expensive backfill later once real schools exist.
  Plain CharField defaulting to "Nigeria", not an enum or a Country model,
  no multi-country logic of any kind exists yet, this is forward
  compatibility only
- institution_type deliberately kept to three values (UNIVERSITY/
  POLYTECHNIC/COLLEGE_OF_EDUCATION) even though source regulator data has
  six raw categories. Allied institutions, colleges of health technology,
  and colleges of nursing all collapse into POLYTECHNIC, the original
  specific label is preserved per-record on SchoolSourceRecord.raw_category,
  nothing is lost, it's just not promoted to a first-class School field
- The actual import command consuming a cleaned NUC/NBTE/NCCE dataset is
  deliberately not built yet, dataset cleaning is happening in a separate
  chat first. This pass only lands the schema, serializers, views, and
  admin needed to receive that data whenever it's ready
- institution_type deliberately kept to three values, not six. Allied
  institutions, colleges of health technology, and colleges of nursing all
  collapse into POLYTECHNIC as the nearest general category (all three are
  NBTE-regulated, sub-degree technical institutions). The original specific
  category label is preserved per-record on SchoolSourceRecord.raw_category,
  nothing from the source is actually lost, it's just not promoted to a
  first-class School field
- country added now, while the directory was still seed-only, specifically
  to avoid a far more expensive backfill later once real schools existed.
  Plain CharField, not an enum or a Country model, no multi-country logic
  built yet, this is forward compatibility only
- Merge philosophy ended deliberately conservative after real, repeated
  false positives during data cleaning. Only two merges are fully
  automated, both gated on an explicit source-provided annotation (an
  institution's own listing stating it was renamed/upgraded). Every other
  candidate merge, including NCCE cross-references and general
  name-similarity duplicates, requires an explicit human decision, applied
  by hand once confirmed, never guessed automatically. Full-string name
  similarity, even at a 0.92+ threshold, was directly observed producing
  false positives (distinct federal colleges in different cities sharing
  a long common template), confirming there's no safe fixed threshold for
  this dataset's naming conventions, not something more tuning fixes
- Newly imported schools default to is_active=False, not True. This is a
  manual QA gate given the known imperfections in the source data (see
  known_possible_duplicates.csv), distinct from Hub activation, a school
  can be reviewed and activated well before anyone requests a hub for it
- seed_demo_data was deliberately NOT modified to scope --clear to
  demo-only schools. Its School.objects.all().delete() still wipes the
  entire table, including the real imported directory. Chosen operational
  discipline instead of a code change: always rerun
  import_school_directory after any seed_demo_data --clear, since the
  import is fully idempotent and safe to rerun at any time regardless of
  School table state. Revisit this if seed_demo_data's clear behavior
  ever causes real friction
