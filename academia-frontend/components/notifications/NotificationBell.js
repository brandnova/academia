...
export default function NotificationBell() {
  ...
  async function handleToggle(wasOpen, toggle) {
    const next = !wasOpen;
    toggle();
    if (next && status === "idle") {
      setStatus("loading");
  ...
  return (
    <Dropdown
      panelClassName="fixed top-16 left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 sm:mt-2 sm:w-80 max-h-[70vh] overflow-auto"
      trigger={({ open, toggle }) => (
        <button
          onClick={() => handleToggle(open, toggle)}
    ...
    >
      {({ close }) => (
        <>
          ...
          {status === "ready" &&
            notifications.map((n) => (
              <NotificationItem key={n.id} notification={n} onRead={handleRead} onNavigate={close} />
            ))}
          <Link
            href="/notifications"
            onClick={close}
